...
# Cache para evitar chamadas repetidas à mesma região na mesma rodada
CACHE_CLIMA: dict[str, dict] = {}

# Mapeamento geográfico aproximado para consultas na API de clima
COORDENADAS_REGIOES = {
...
def buscar_mapeamento_rbc(mac_adress: list[str]) -> pd.DataFrame:
    ...
    try:
        ...
        sql = """
            SELECT
                ...
                CONCAT('Linha ', l.numeroLinha) AS nome_linha,
                r.idRbc AS id_rbc,
    ...
    # Coluna regiao: será substituída pelo valor real do banco quando disponível
...
def caminho_para_tratado(df: pd.DataFrame, tipo: str) -> str:
    ...
    # ── client/ ───────────────────────────────────────────────────────────
        ...
        return f"client/{nome_empresa}/{mac_adress}/latest.json"
    elif tipo == "semanal-json":
        return f"client/{nome_empresa}/{mac_adress}/semanal.json"

    elif tipo == "incidentes":
        return f"trusted/{nome_empresa}/{ano}/{mes}/{dia}/incidentes/incidentes_{ano}-{mes}-{dia}.csv"
...
def salvar_csv_trusted(df: pd.DataFrame, bucket: str, tipo: str):
    ...
    s3 = boto3.client("s3", **cfg_s3())
    print(f"Enviando CSV para s3://{bucket}/{caminho} ...")
...
# ---------------------------------------------------------------------------
# Climatempo / Incidentes
# ---------------------------------------------------------------------------

def obter_clima_por_regiao(regiao: str) -> Optional[dict]:
    """Busca o clima atualizado por coordenadas usando a região do RBC como chave."""
    ...
    try:
        ...
        if lat and lon:
            ...
            # Estações fora de SP (Osasco, Suzano, Mogi…)
...
def processar_e_salvar_incidentes_csv(df_tratado: pd.DataFrame, df_alertas: pd.DataFrame, bucket: str):
    ...
    enriquece geograficamente via OpenWeather por coordenadas regionais e grava em CSV por dia.
    """
    print("\n" + "=" * 50)
    ...
    for row in df_offline.itertuples():
        regiao_servidor = getattr(row, "regiao", "CENTRO")
        ...
        incidentes_lista.append({
            ...
            "tipo_incidente": "OFFLINE", 
            "criticidade": "CRÍTICO",
            "componente_afetado": "CONEXÃO",
            "descricao": "Servidor perdeu a comunicação com a central de telemetria.",
    ...
    if df_alertas is not None and not df_alertas.empty:
        ...
        for row in df_criticos.itertuples():
            registro_orig = df_tratado[
                (df_tratado["endereco_mac"] == row.endereco_mac) &
                (df_tratado["data_hora_iso"] == row.data_hora_iso)
            ]

            regiao_servidor = (
                registro_orig["regiao"].iloc[0]
                if (not registro_orig.empty and "regiao" in registro_orig.columns)
                else "CENTRO"
            )
            info_clima = obter_clima_por_regiao(regiao_servidor)
            ...
            incidentes_lista.append({
                ...
                "componente_afetado": str(row.componente_afetado).upper(),
                "descricao": str(row.motivo_alerta).strip(),
    ...
    if not incidentes_lista:
        print("[INCIDENTES] Sem ocorrências críticas para esta execução.")
        return
    ...
    if df_existente is not None:
        ...
        df_final.drop_duplicates(
            subset=["id_rbc", "componente_afetado", "data_hora_evento"],
    ...
    try:
        ...
        s3_client.put_object(
            Bucket=bucket, Key=caminho_s3,
            Body=buffer.getvalue(), ContentType="text/csv",
        )
        print(f"[INCIDENTES] Upload concluído: {len(df_novos_incidentes)} linhas → s3://{bucket}/{caminho_s3}")
    except Exception as e:
...
def processar_arquivo_s3(bucket: str, key: str) -> dict:
    ...
    if df_alertas is not None and not df_alertas.empty:
        ...
        processar_e_salvar_incidentes_csv(
            df_tratado=df,
