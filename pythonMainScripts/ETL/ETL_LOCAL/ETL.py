...
#       bibliotecas (maior parte nativa ou que tu já conhece(provavel(talvez)))
#       config feita
#       cria caminho e pasta baseada na config
#       lê dataframe procura datetime e macadress
#       associa com mysql
#       cria valores humanos p/leitura
#       separa a string de processos em uma lista de dicts real
#       cria os dataframe maquina | empresa_maquina | processos_empresa_maquina
#       cria df simples de correlação para previsão
#       cria um csv para cada
#       cria um csv para maquina com melhor informação
#       cria um csv grande demais para processos totais

# NÃO FIZ COACTAR  NO S3 E OU EC2 
# MODIFIQUEI LEVEMENTE O MYSQL PARA FACILITAR MINHA VIDA
# ESSA VESAO NÃO É OFICIAL. 
# GUD LOCK
...
def export_all(
    ...
    empresas: Dict[str, pd.DataFrame],
    maquinas: Dict[Tuple, pd.DataFrame],
    processos_por_maquina: Dict[Tuple, pd.DataFrame],
) -> None:
...
def main():
    ...
    print(f"MACs únicos encontrados no CSV: {len(unique_macs)}")
    ...
    print(f"Arquivos gerados em: {Path(OUTPUT_DIR).resolve()}")
